...
import { DocumentPreviewCard } from "../../_components/DocumentPreviewCard";
import { 
    Dialog, 
    DialogContent, 
    DialogHeader, 
...
type ServiceItem = {
    ...
    accreditation_files_metadata?: Document[];
    latitude?: number;
    longitude?: number;
};
...
export default function ReviewPage() {
    ...
    const [viewingDoc, setViewingDoc] = useState<{ doc: Document, contextId: string, contextType: 'profile' | 'service' } | null>(null);
             ...
             <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="flex items-center gap-5">
                    <Button 
                        ...
                        className="rounded-full h-10 w-10 bg-white shadow-sm border border-serene-neutral-200 hover:bg-serene-neutral-50"
                    >
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <div>
                        <h1 className="text-3xl font-bold text-serene-neutral-900 tracking-tight flex items-center gap-3">
                            {profile.first_name} {profile.last_name}
                            <Badge variant="secondary" className="text-sm font-normal bg-serene-neutral-100 text-serene-neutral-600 capitalize">
                                {profile.user_type}
                            </Badge>
                        </h1>
                        <div className="flex items-center gap-3 text-serene-neutral-500 text-sm mt-1.5 font-medium">
                           <span>Joined {new Date(profile.created_at).toLocaleDateString()}</span>
                           <span className="h-1 w-1 rounded-full bg-serene-neutral-300"></span>
                           <span className={cn(
                                ...
                                {profile.verification_status === 'verified' ? <CheckCircle className="h-3.5 w-3.5" /> :
                                 profile.verification_status === 'rejected' ? <XCircle className="h-3.5 w-3.5" /> :
                             ...
                             <Card className="border-serene-neutral-200 shadow-sm rounded-3xl overflow-hidden bg-white">
                                ...
                                <CardContent className="space-y-6 pt-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <InfoBlock label="Professional Title" value={profile.professional_title} />
                                        <InfoBlock label="Bio" value={profile.bio} fullWidth />
                                    </div>
                                    ...
                                    {services.map(service => (
                                        <AccordionItem key={service.id} value={service.id} className={cn(
                                            ...
                                            <AccordionContent className="pb-6 pt-2 px-4 border-t border-black/5 mt-2">
                                                 <div className="flex flex-col xl:flex-row justify-between items-start gap-8 pt-4">
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 flex-1 w-full">
                                                        <InfoBlock label="Helpline" value={service.helpline} />
                                                        <InfoBlock label="Website" value={service.website} />
                                                        <InfoBlock 
                                                        ...
                                                        />
                                                        <InfoBlock label="Service Types" value={Array.isArray(service.service_types) ? service.service_types.join(', ') : service.service_types} />
                                                        <InfoBlock label="Description" value={service.description} fullWidth />
                                                    </div>
...
// Subcomponents
function InfoBlock({ label, value, fullWidth = false }: { label: string, value: any, fullWidth?: boolean }) {
    return (
        <div className={cn(fullWidth ? "col-span-full" : "")}>
            ...
            <span className="text-sm font-medium text-gray-900 block break-words">{value || "-"}</span>
        </div>
    );
}

function DocumentsGrid({ documents, onView }: { documents: Document[], onView: (doc: Document) => void }) {
    if (documents.length === 0) return <p className="text-sm text-gray-500 italic">No documents attached.</p>;
...
function ActionButtons({ status, onAction, size = "default" }: { status: string, onAction: (action: 'verify' | 'reject' | 'ban') => void, size?: "default" | "sm" }) {
...
const parseDocuments = (filesJson: any, metadataJson: any): Document[] => {
    // Prefer metadata as it has review status and extended info
    if (metadataJson) {
         try {
            const parsedMeta = typeof metadataJson === 'string' ? JSON.parse(metadataJson) : metadataJson;
            if (Array.isArray(parsedMeta) && parsedMeta.length > 0) {
                return parsedMeta.map((m: any) => ({
                    url: m.url,
                    name: m.title || m.name || m.fileName || 'Document', // Normalized name
                    type: m.fileType || m.type || 'unknown',
                    status: m.status,
                    notes: m.notes,
                    docNumber: m.docNumber, // Pass through extended fields
                    issuer: m.issuer,
    ...
    // Fallback to basic files parsing if metadata is missing/empty
    if (filesJson) {
         try {
            const parsedFiles = typeof filesJson === 'string' ? JSON.parse(filesJson) : filesJson;
            if (Array.isArray(parsedFiles)) {
                return parsedFiles.map((f: any) => ({
                    url: typeof f === 'string' ? f : (f?.url || ''),
                    name: typeof f === 'string' ? 'Document' : (f?.name || 'Document'),
                    type: typeof f === 'string' ? 'unknown' : (f?.type || 'unknown')
                })).filter(d => d.url);
            }
         } catch (e) {
