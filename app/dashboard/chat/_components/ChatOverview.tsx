...
export function ChatOverview({
	...
	const [activeTab, setActiveTab] = useState<"community" | "dm" | "contacts">(
		"community"
	);
						...
						{activeTab === "dm" ? (
							<div className="flex-1 overflow-y-auto">
								{dmChannels.map((ch: any) => {
									...
									const other = (
										members.find((m: any) => m?.user?.id && m.user.id !== userId) as any
									)?.user;
									...
									return (
										<button
											...
											onClick={() => router.push(`/dashboard/chat/c/${ch.id}`)}
										>
										...
										<div className="flex items-center justify-between p-2 bg-white/90 backdrop-blur border-b border-gray-200">
											...
											<div className="flex items-center gap-2">
												<Button
													size="sm"
													variant="outline"
													onClick={() => router.push("/dashboard/chat/community")}
												>
													Open
												</Button>
											</div>
								...
								{activeTab === "dm" ? (
									dmChannels.length ? (
										...
										<div className="flex flex-col items-center justify-center py-20 px-4">
											<div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-purple-50 to-pink-100 flex items-center justify-center">
												<MessageSquare className="h-10 w-10 text-purple-500" />
											</div>
											<h3 className="text-xl font-semibold text-gray-900 mb-3">
												No messages yet
											</h3>
											<p className="text-gray-500 max-w-md mx-auto leading-relaxed text-center">
												Start a conversation with a professional or survivor to begin
												chatting.
											</p>
										</div>
									)
