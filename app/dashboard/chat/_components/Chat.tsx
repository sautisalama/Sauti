	...
	useEffect(() => {
		try {
			...
			window.dispatchEvent(
				new CustomEvent("ss:chat-active", { detail: { active } })
			);
		} catch {}
		return () => {
			try {
				window.dispatchEvent(
					new CustomEvent("ss:chat-active", { detail: { active: false } })
				);
			} catch {}
		};
		...
		const initChat = async () => {
			try {
				...
				if (pre && !client) {
					console.log(
						`Using preloaded ${
							pre.isAnonymous ? "anonymous" : "regular"
						} chat data - instant load!`
					);
					...
					if (pre && pre.isAnonymous !== currentAnonymous) {
						console.log(
							`Mode changed to ${
								currentAnonymous ? "anonymous" : "regular"
							}, switching client...`
						);
								...
								<div className="mt-2">
									<div className="flex items-center gap-2 border rounded px-2 py-1.5">
										...
										<input
											...
											placeholder={showNewChat ? "Search contacts" : "Search"}
										...
										{(
											...
											.filter((ch: any) => {
												...
												if (ch?.type === "livestream") label = "Sauti Community";
													...
													const other = (
														members.find(
															(m: any) => m?.user?.id && m.user.id !== userId
														) as any
													)?.user;
											...
											.map((ch: any) => {
												...
												const last = ch?.state?.messages
													?.slice()
													.reverse()
													.find((m: any) => !m.deleted_at);
												const label = isCommunity
													? "Sauti Community"
													: other?.name || other?.id || "Unknown";
								...
								<div className="w-full h-full flex flex-col min-h-0">
											...
											<div className="flex items-center justify-between p-2 bg-white/90 backdrop-blur border-b border-gray-200">
												...
												<div className="flex items-center gap-2">
													<Button
														...
														onClick={() => collectMedia(channel)}
													>
														<Images className="h-4 w-4 mr-1" /> Media{" "}
														{mediaCounts.attachments ? `(${mediaCounts.attachments})` : ""}
													</Button>
													<Button
														size="sm"
														variant="outline"
														className="hidden sm:inline-flex"
														onClick={() => collectLinks(channel)}
													>
														<LinkIcon className="h-4 w-4 mr-1" /> Links{" "}
														{mediaCounts.links ? `(${mediaCounts.links})` : ""}
													</Button>
													<Button
														size="sm"
														variant="outline"
														...
														return (
															<>
																...
																<Button
																	...
																	onClick={async () => {
																		try {
																			await client?.banUser(other!, { timeout: 60 * 24 });
																		} catch (e) {
																			console.warn("banUser failed", e);
																		}
																	}}
																>
																	Block 24h
									...
									<AppointmentScheduler
										...
										onSchedule={async (date) => {
											...
											if (!me || !other) return;
											const isPro =
												me.user_type === "professional" || me.user_type === "ngo";
											const appointment = {
