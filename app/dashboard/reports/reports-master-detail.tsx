...
import { CalendarDays, User, Phone, Mail, Clock, FileText, ChevronRight, Paperclip, Shield, X, ChevronLeft } from "lucide-react";
...
interface ReportItem extends Tables<"reports"> {
  matched_services?: Array<{
    ...
    support_services: { id: string; name: string; phone_number?: string | null; email?: string | null };
...
export default function ReportsMasterDetail({ userId }: { userId: string }) {
    ...
    const load = async () => {
      try {
        ...
        const normalized = (data as any)?.map((r: any) => ({
          ...r,
          matched_services: r.matched_services?.map((m: any) => ({
            ...m,
            support_services: m.support_service || m.support_services || null,
          })) || [],
        }));
        setReports(normalized || []);
        try { localStorage.setItem(`reports-cache-${userId}`, JSON.stringify(normalized || [])); } catch {}
      } catch (e: any) {
  ...
  const selected = useMemo(() => filtered.find(r => r.report_id === selectedId) || null, [filtered, selectedId]);
    ...
    <div className={`grid grid-cols-1 lg:grid-cols-12 gap-4 ${selected ? 'overflow-x-hidden' : ''}`}>
      ...
      <div className="lg:hidden -mt-1 mb-1">
        <div className="inline-flex rounded-lg border bg-white p-1">
          <button onClick={() => setMobileView("list")} className={`px-3 py-1.5 text-xs font-medium rounded-md ${mobileView === 'list' ? 'bg-[#1A3434] text-white' : 'text-neutral-700'}`}>Reports</button>
      ...
      <div className={`lg:col-span-7 xl:col-span-7 ${mobileView !== 'list' ? 'hidden lg:block' : ''}`}>
        <div className="mb-3 flex items-center gap-2">
          <Input placeholder="Search reports (incident, description, urgency)" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        <div className="space-y-2">
          {filtered.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">No reports to display.</div>
          )}
          {filtered.map((r) => {
            ...
            return (
              <button 
                ...
                className={`w-full text-left rounded-lg border p-3 bg-white hover:shadow transition-shadow ${isActive ? "ring-2 ring-[#1A3434]" : ""}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-medium truncate">{r.type_of_incident || "Unknown Incident"}</h3>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${urgencyColor(r.urgency)}`}>{r.urgency || "low"}</span>
                    </div>
                    {r.incident_description && (
                      <p className="text-xs text-neutral-600 mt-1 line-clamp-2">{r.incident_description}</p>
                    )}
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
                      ...
                      {ms && (
                        <>
                          ...
                          <span className="px-2 py-0.5 rounded-full bg-neutral-100 border text-neutral-700 truncate">{ms.support_services?.name || ms.match_status_type}</span>
                        </>
                      )}
                      {appt && (
                        <>
          ...
          <UIDateCalendar
            ...
            onSelect={(date) => {
              ...
              const firstWithAppt = filtered.find((r) =>
                (r.matched_services || []).some((m) => (m.appointments || []).some((a) => new Date(a.appointment_date).toDateString() === dayKey))
              );
      ...
      <div
        className={`fixed inset-0 sm:inset-y-0 sm:right-0 sm:left-auto w-full sm:w-[540px] lg:w-[720px] bg-white shadow-2xl border-l z-40 transform transition-transform duration-300 ease-out ${selected ? 'translate-y-0 sm:translate-x-0' : 'translate-y-full sm:translate-x-full'}`}
        aria-hidden={!selected}
        style={{ transform: selected ? (dragY ? `translateY(${dragY}px)` : undefined) : undefined }}
          ...
          <div className="h-full flex flex-col">
            ...
            <div className="p-4 border-b flex items-start justify-between gap-3 sticky top-0 bg-white z-10">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-semibold text-[#1A3434]">{selected.type_of_incident || "Unknown Incident"}</h2>
                  <span className={`px-2 py-0.5 rounded-full text-xs ${urgencyColor(selected.urgency)}`}>{selected.urgency || "low"}</span>
                </div>
                <div className="mt-1 text-xs text-neutral-600 flex items-center gap-2">
                  <Shield className="h-3 w-3" /> Report ID: {selected.report_id}
                  <span>•</span>
                  <Clock className="h-3 w-3" /> {formatDate(selected.submission_timestamp)}
                </div>
              </div>
              ...
              <div className="lg:col-span-3 space-y-4">
                ...
                {selected.incident_description && (
                  <div className="p-4 rounded-lg bg-neutral-50 border">
                    ...
                    <p className="text-sm text-neutral-800 whitespace-pre-wrap">{selected.incident_description}</p>
                  </div>
                )}
                ...
                <RichTextNotesEditor
                  ...
                  report={selected}
                  onSaved={(updated: any) => {
                    const next = reports.map(r => r.report_id === updated.report_id ? (updated as any) : r);
                    setReports(next);
                    try { localStorage.setItem(`reports-cache-${userId}`, JSON.stringify(next)); } catch {}
                  }}
                />

                {/* Attachments (if any in administrative JSON) */}
                {(() => {
                  const attachments = (selected as any)?.administrative?.attachments as Array<{ name: string; url: string }>|undefined;
                  if (!attachments || attachments.length === 0) return null;
                  return (
                    <div className="p-4 rounded-lg bg-neutral-50 border">
                      <h3 className="text-sm font-medium text-neutral-700 mb-2 flex items-center gap-2"><Paperclip className="h-4 w-4" />Attachments</h3>
                      <ul className="space-y-2 text-sm">
                        {attachments.map((a, idx) => (
                          <li key={idx} className="flex items-center justify-between">
                            <span className="truncate flex items-center gap-2"><FileText className="h-4 w-4 text-neutral-500" /> {a.name}</span>
                            <a href={a.url} target="_blank" rel="noreferrer" className="text-sauti-orange text-xs">Open</a>
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {selected.matched_services && selected.matched_services.length > 0 ? (
                  <div className="p-4 rounded-lg bg-neutral-50 border">
                    ...
                    <p className="text-sm text-neutral-800">{selected.matched_services[0].support_services?.name || "Pending Match"}</p>
                    <p className="text-xs text-neutral-500">Status: {String(selected.matched_services[0].match_status_type)}</p>
                    <div className="mt-3">
                    ...
                    return (
                      <div className="p-4 rounded-lg bg-neutral-50 border">
                        ...
                        <div className="text-sm text-neutral-800 flex items-center gap-2">
                          <CalendarDays className="h-4 w-4" /> {new Date(appt.appointment_date).toLocaleString()}
                        </div>
                        <div className="mt-1">
          ...
          {selected?.matched_services?.[0] ? (
            <div className="space-y-3">
              <div className="p-3 rounded-lg bg-neutral-50 border">
                <h4 className="text-sm font-medium text-neutral-800">{selected.matched_services[0].support_services?.name}</h4>
                <div className="text-xs text-neutral-600 mt-1 flex items-center gap-2">
                  <User className="h-3 w-3" /> {selected.matched_services[0].appointments?.[0]?.professional?.first_name} {selected.matched_services[0].appointments?.[0]?.professional?.last_name}
                </div>
                {selected.matched_services[0].support_services?.phone_number && (
                  <div className="text-xs text-neutral-600 mt-1 flex items-center gap-2">
                    <Phone className="h-3 w-3" /> {selected.matched_services[0].support_services?.phone_number}
                  </div>
                )}
                {selected.matched_services[0].support_services?.email && (
                  <div className="text-xs text-neutral-600 mt-1 flex items-center gap-2">
                    <Mail className="h-3 w-3" /> {selected.matched_services[0].support_services?.email}
                  </div>
                )}
              </div>
              <div className="text-xs text-neutral-500">
                Reach out to your matched professional if you need to adjust scheduling or discuss details.
              </div>
            </div>
          ) : (
            <div className="text-sm text-neutral-500">No matched professional found.</div>
          )}
        </DialogContent>
      </Dialog>
    </div>
